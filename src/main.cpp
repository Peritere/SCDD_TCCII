/* Programa para monitoramento de Extrusoras 

    Criação data: 04-09-2023

    Autor:
     -Pericles Silva

    Versão Arduino Beta 2.9.0
    // PULSOS = 1-BATIDA = 1 VOLTA COMPLETA NO ROTOR 360º
*/


#include <SPI.h> //(Paul Stoffregen). 
#include <Ethernet.h> //  (Arduino-libraries). 
#include <SdFat.h> //   (Arduino Reference). 
#include <DS3232RTC.h> // https://github.com/JChristensen/DS3232RTC
#include "RTCDS1307.h" //  Relógio interno. (Arduino-libraries). 
#include <ArduinoJson.h> // (Arduino Reference). 
#include <ArduinoHttpClient.h>  // (Arduino Reference).
#include <avr/wdt.h> // for self reset
#include "EEPROM.h" //  (Arduino Reference).
...
//                                                    #
int nTeste    = 0;   // 0 = Normal -  1 = Teste       # FUNCÃO ARITMÉTICA
int nVelTeste = 3.6; // FORMULA PARA O RPM            # 
//                                                    #
//                                                    #
//#####################################################
...
String cTurnoBInicio = "14:10:01"; //   ALTERE CONFORME NECESSIDADE
...
void Procura_Dados(); // ABRE AS INFORMAÇÕES DO SD.
void Guarda_Dados();  // ARMAZENA DADOS

void contador() // VARIAVEL PARA ARMAZENAR OS PULSOS DA TRANSFORMAÇÃO DO RPM
{
  // Incrementa contador
  nFalhas = 0;    // Reseta as falhas
  variacao = 800; // Fala ao programa que há pulsos na porta Digital
  pulsos++;
}
...
void botaoAcionado() // monitora o tempo de inicio e fim do rpm
{
  nBatidas++;
  nBatidasMes++;
  nBatidasOP++;
  ...
  if (millis() - timeold >= 1000)
  {
    ...
    noInterrupts();
    nRPM = (60 * 1000 / pulsos_por_volta) / (millis() - timeold) * pulsos; // executa a operação aritmetica da transf. do rpm
    pulsos = 0;
//    Serial.print("RPM = ");
//    Serial.println(nRPM);
...
void LeituraMaquina()
{
  ...
  CarregaDataHora();
  delay(1);
///*****************************************ATENSÃO USAR SE TIVER CERTEZA POIS PODE LIGAR A MÁQUINA REMOTO********************
/*                                  
...
void setup()
{
  ...
  Serial.println("############################");
  Serial.println("|                          |");
  Serial.println("|  Extrusora_2.9." + String(VERSION) + "  |");
  Serial.println("|                          |");
  Serial.println("############################");
...
void reiniciarRobocore()
{
  ...
  Serial.println("REINICIANDO AGUARDE...");
  delay(5000);
...
void loop()
{
  ...
  /*                                          ATIVAR SOMENTE SE QUISER USAR BOTÃO LOCAL/ BOTAO DE RESET
    //Botao  
          ...
          if (readString.indexOf("/Listar") > 0)
          {
            ...
            if (readString.substring(Posicao1, Posicao2) == "Listar")
            {
              ...
              if(myFile){
                ...
                Serial.println("Data.txt não existe");
              }

              //Gera arquivo DataRead.txt caso não exista
              if (!SD.exists("DataRead.txt"))
              {
                if (myFile)
                {
                  ...
                  myFile.close();
                  bool bisDataRenomeado = SD.rename("Data.txt", "DataRead.txt");


                  Serial.print("Renomeando Data.txt para DataRead.txt, status:");
                  Serial.println(bisDataRenomeado);
                  ...
                  if(bisDataRenomeado) {
                    ...
                    Serial.println("Arquivo não renomeado DataRead.txt");
              ...
              {
                myFile2 = SD.open("DataRead.txt"); // abre o arquivo da pagina WEB
                Serial.println("DataRead.txt existe, não foi necessario criar");
              }
...
//                      LeituraMaquina();
          ...
          if (readString.indexOf("/CARREGAOP1") > 0)
          {
            ...
            if (readString.substring(Posicao1, Posicao2) == "CARREGAOP1")
            {
              ...
              nBatidasBobina = 0; //NUMERO DE PULSOS
              nBatidasOP = 0;
            ...
            if (readString.substring(Posicao1, Posicao2) == "CARREGAOP2")
            {
              ...
              if (statusCode == 200)
              {
                ...
                else
                {
                  ...
                  nBatidasBobina = 0; // BATIDAS == PULSOS DOS SENSORES
                  nBatidasOP = 0;
          ...
          {                                                            // Zera os valores abaixo
            ...
            if (readString.substring(Posicao1, Posicao2) == "VERSAO")
            {
              ...
              client.println("    |                                     |");
              client.println("    |       -----------------------       |");
              client.println("    |           VERSAO_FULL_2.9." + String(VERSION) + "       |");
              client.println("    |       -----------------------       |");
              client.println("    |                                     |");
